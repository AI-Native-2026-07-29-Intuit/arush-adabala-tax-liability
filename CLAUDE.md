...
1. Target jdk 17 or above. Reserve var for a few places and use it only when it makes sense (make it easier to read) – otherwise opt for explicit type in public API.
...
4. Dates: java.time.LocalDate for calendar dates (when income was recorded), java.time.Instant for timestamps. Avoid using java.util.Date, java.sql.Date, or java.util.Calendar.

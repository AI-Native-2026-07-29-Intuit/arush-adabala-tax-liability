...
helm upgrade --install kube-prometheus-stack prometheus-community/kube-prometheus-stack \
  ...
  -f "${VALUES}/kube-prometheus-stack.values.yaml" --wait --timeout 10m

echo "[2/5] loki ${LOKI_VERSION}"
helm upgrade --install loki grafana/loki \
  --version "${LOKI_VERSION}" --namespace "${NS}" \
  -f "${VALUES}/loki.values.yaml" --wait --timeout 10m

echo "[3/5] tempo ${TEMPO_VERSION}"
helm upgrade --install tempo grafana/tempo \
  --version "${TEMPO_VERSION}" --namespace "${NS}" \
  -f "${VALUES}/tempo.values.yaml" --wait --timeout 10m

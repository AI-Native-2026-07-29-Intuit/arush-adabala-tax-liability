...
#
# WHAT TO WATCH, AND THE ONE THING THAT SURPRISES PEOPLE
#
...
#
# So a freshly deployed worker sits at 1 replica with ACTIVE=True and an empty topic, which reads
# exactly like "KEDA thinks there is work when there is none". There is no bug. Produce once, let
# the group commit, and the offsets become valid - from then on lag is real and scale-to-zero
# works. This script is what gets it over that line the first time.
set -euo pipefail
...
$KUBECTL -n "${NS}" exec -i deploy/kafka -- bash -c "
  ...
  for r in \$(seq 1 ${REPEATS}); do cat /tmp/w6d5-batch.json; done \

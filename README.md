...
- **The Java/Python round-trip, and the one place the two do not line up.** The fixture at
  `taxcalc-ai/tests/fixtures/taxpayer_java.json` is real Jackson output, generated by serialising
  this project's own compiled `TaxpayerReadModel` class rather than hand-written to match what
  Python happens to emit. It exposes a genuine seam: Java writes `BigDecimal` money as a JSON
  **number** with its scale preserved (`120000.00`), Pydantic re-emits a `Decimal` as a JSON
  **string**, and a JSON number's trailing zeros survive neither parser. Byte-equality is
  therefore unachievable without faking one side of the fixture, so the test asserts the contract
  that matters — key sets match in both directions, and re-validating our own output reproduces
  an equal model. The real fix (`@JsonFormat(shape = STRING)` on the Java money fields) is
  written up in `PYTHON.md` as a follow-up that deserves its own PR, because it changes a wire
  format the W4 React client and the W5 D4 Lambda both already parse.

...
**`infra/oidc/`** holds the committed, reproducible shape of both IAM trust policies (`trust-policy-build.json`, `trust-policy-prod.json`) and `scripts/docker-oidc-bootstrap.sh` (sibling to the existing `scripts/oidc-bootstrap.sh`, same structure) applies them against a real AWS account. Both files use the textbook `repo:<org>/<repo>:...` subject-claim form as a *documented default*, not a verified value: `oidc-bootstrap.sh` already found, for this exact organization, that GitHub's real OIDC token embeds internal numeric org/repo ids instead (`repo:ORG@309728071/REPO@1317703842:...`) — a policy trusting the textbook form would fail with an opaque `Not authorized to perform sts:AssumeRoleWithWebIdentity`, no mention of `sub` anywhere. `infra/oidc/README.md` carries the full warning and the fix (confirm the real claim from a live token before applying). The build role's trust policy pins `sub` to *both* `environment:dev` and `ref:refs/heads/main` (belt-and-suspenders); the prod role pins to `environment:prod` **only** — deliberately no branch condition, since `workflow_dispatch` can run from any branch a caller can push to, and the Environment's required-reviewer gate (not a `sub` string) is what actually decides whether the run starts.
...
**Verified live**, not just read: `actionlint` (run via the pinned `rhysd/actionlint` container, matching this repo's existing preference for pinned containers over ad-hoc downloaded binaries — see W5 D5's Sloth section) reports zero findings on every new file beyond the one pre-existing, already-expected `blacksmith-2vcpu-ubuntu-2204 is unknown` runner-label warning every other workflow in this repo also carries (actionlint's built-in label list has no concept of a self-hosted-class runner name unless configured, which none of this repo's workflows do). `ruby -ryaml` round-trip-parses every new YAML file. Both trust-policy JSONs parse cleanly with Python's `json.load`. `shellcheck` is clean on `docker-oidc-bootstrap.sh`. The PR's own `Build + test (taxcalc-api)` check went green end to end against the real Testcontainers suite (~5 minutes), and `call-build-and-push` correctly reported `skipping` on the PR event (it only fires on push to `main`).

**Also done, not just written:** the `dev` and `prod` GitHub Environments now exist for real (created via the GitHub API), each with its deployment-branch policy restricted to `main` only. **One piece is a genuine, flagged gap:** adding `prod`'s required-reviewer rule is a permission-granting repo-admin action this session's own safety guardrails declined to take unattended — which happens to match the lesson's own framing exactly ("the UI configuration is not in version control... screenshot it for the PR"). `.github/PIPELINE.md`'s "What is NOT in this repo" section has the two-line manual follow-up (add the required reviewer; add `main`'s branch-protection rule requiring the `build-test` check once it's gone green at least once — which, per the paragraph above, it now has).

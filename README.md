...
- **Task 2** — `kubectl describe pod` showed all three probes at the exact configured paths/periods/thresholds; `kubectl describe secret` listed the key with no value; readiness genuinely reflected Postgres/Mongo health (`show-details: always`), not just a heartbeat.
- **Task 3** — the HPA reported real metrics-server-backed CPU numbers; a 90s/50-concurrency load run against the Service drove CPU to 106%/70% target and scaled the Deployment 3 → 5; curling through the Ingress with the `Host` header reached the same readiness endpoint as a direct Service curl.
- **Task 4** — bumped `0.1.0` → `0.1.1`, ran a steady-state readiness curl loop through the rollout: all 128 requests returned `200`, confirming `maxUnavailable: 0` + the readiness probe + the EndpointSlice contract actually holds end-to-end. Then pushed a deliberately-broken `0.1.2` image (an `nginx-unprivileged`-based stand-in whose `/actuator/health/readiness` always returns `503` — no need to touch the real Spring app to prove the rollout mechanics), watched `kubectl rollout status` correctly time out mid-rollout with the old ReplicaSet still serving all traffic, then `kubectl rollout undo` cleanly restored `0.1.1` — `kubectl rollout history` shows 5 real revisions from this session.

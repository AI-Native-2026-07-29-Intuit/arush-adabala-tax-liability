...
**What it did not close, with the evidence for each — these still need real AWS:**

| Graded artefact | What floci actually reported |
|---|---|
| SnapStart + cold-vs-warm latency | `get-function-configuration` → `SnapStart: {ApplyOn: None, OptimizationStatus: Off}` despite the template requesting `PublishedVersions`, and alias `live` resolves to `$LATEST` rather than a published version. The emulator has no Firecracker snapshot/restore, so **any latency table taken here would measure Docker container start, not SnapStart** — it is deliberately not recorded. |
| IAM least-privilege | The transform did **not** expand `DynamoDBReadPolicy`: the generated role carries only `AWSLambdaBasicExecutionRole`, with `Policies: null` in the processed template and no inline policies. The function then read DynamoDB successfully anyway, which proves floci does not *enforce* IAM either. Unverifiable twice over. |
| `LogFormat: JSON` envelopes | Template says `JSON`; the provisioned function reports `LoggingConfig.LogFormat: Text`. The advanced-logging-controls behaviour is simply not emulated. |
| EMF → custom metric | `list-metrics --namespace TaxcalcDev` → `{"Metrics": []}`. The EMF line is stored as an ordinary log line and never parsed, so **the open question from the previous section — whether `_aws` still gets extracted once ALC wraps stdout in a JSON envelope — remains open.** |
| p99 alarm | Alarm created, but `ExtendedStatistic` came back `None` (the p99 was dropped) and the state is `INSUFFICIENT_DATA`. |
| CloudWatch `REPORT` lines | None emitted — those come from the real Lambda runtime, not the function. Application log lines *do* reach the log group. |
| OIDC CI deploy | Not applicable at any emulator: it is GitHub↔AWS STS trust plus a real Actions run. |

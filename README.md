...
2. **`sam build --use-container` also stopped failing, for free.** Behind this laptop's Zscaler TLS interception, a containerised Maven build could not reach Maven Central (the container truststore has no corporate root CA), so `--use-container` failed here while plain `sam build` succeeded. With `SkipBuild` there is no Maven run inside the container at all — it only stages an already-built artefact — so all three of `sam validate --lint`, `sam build` and `sam build --use-container` now exit 0 on this machine. That is not a fidelity loss for this function: a pure-Java shaded jar is byte-identical whether compiled on the host or in the parity image, since `--use-container` exists to correct for *native* extensions.

...
**So all four stacks were deployed against [floci](https://github.com/floci-io/floci) 2.0.1 instead** — the same local emulator W5 D4 and W6 D1 used for exactly this gap. Endpoint only; no template, parameter or command changed. All four reached `CREATE_COMPLETE` through the real `create-change-set → describe-change-set → execute-change-set` flow. **`INFRA.md` says which engine produced every row of evidence**, because two of the four blocked items close and two do not.
...
Two smaller gaps, both isolated rather than assumed. **`validate-template` is a stub**: it returns empty `Parameters` for templates declaring 2–5 of them, and passes a template with a fictional resource type, a dangling `!Ref` and a malformed `!GetAtt` that `cfn-lint` rejects with `E3006` — which is the argument for leaving cfn-lint and cfn-nag ungated in CI and letting only `validate-template` skip. And **`Fn::Split` does not resolve into a list-typed property**, so `DbSubnetGroup` failed; three probes localised it (a literal list works, the same `!Split` + `!ImportValue` in an *Output* works, only `SubnetIds` fails), so the committed template is **unchanged** — that spelling is the documented AWS pattern. `detect-stack-drift` and its two companions return `UnknownAction`, so the drift Done-When has no local path at all.
